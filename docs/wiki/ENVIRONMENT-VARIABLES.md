...
| `PIXELS_I18N_OVERLAY_PATH` | empty | Optional managed translation overlay path or HTTP(S) URL |
| `PIXELS_I18N_OVERLAY_API_KEY` | empty | Optional `X-API-Key` used to read the managed overlay |
...
| `STORAGE_BUCKET` | `pixels-camera` | Durable camera object bucket |
| `STORAGE_DEBUG_BUCKET` | `pixels-debug` | Staff traces and future diagnostic object bucket |
| `STORAGE_DEBUG_PUBLIC_BASE_URL` | empty | Permanent public debug bucket origin override |
