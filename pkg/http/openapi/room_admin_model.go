...
type RoomResponse struct {
	...
	// IsBundleTemplate reports whether the room is a catalog bundle source.
	IsBundleTemplate bool `json:"isBundleTemplate" required:"true"`
...
// RoomRollerSettingsRequest documents an optimistic room roller cadence update.
type RoomRollerSettingsRequest struct {
	RoomIDRequest
	...
	// RollerSpeed stores cycles between steps, or -1 to disable rollers.
	RollerSpeed int `json:"rollerSpeed" required:"true" minimum:"-1" maximum:"20"`
}
...
// RoomPromotionResponse documents one active room event advertisement.
type RoomPromotionResponse struct {
	// ID identifies the promotion.
	ID int64 `json:"id" required:"true"`
	// RoomID identifies the promoted room.
	RoomID int64 `json:"roomId" required:"true"`
	// CategoryID identifies the event category.
	CategoryID int32 `json:"categoryId" required:"true"`
	// Title stores visible event copy.
	Title string `json:"title" required:"true"`
	// Description stores visible event copy.
	Description string `json:"description" required:"true"`
	// StartsAt stores the RFC3339 activation boundary.
	StartsAt string `json:"startsAt" required:"true"`
	// EndsAt stores the RFC3339 expiration boundary.
	EndsAt string `json:"endsAt" required:"true"`
	// CreatedBy identifies the purchaser.
	CreatedBy int64 `json:"createdBy" required:"true"`
	// Version stores mutation order.
	Version int64 `json:"version" required:"true" minimum:"1"`
}
