...
type AdminPlayerUsernameRequest struct {
	...
	Username string `path:"username" required:"true" minLength:"3" maxLength:"15" example:"alice"`
	// IfNoneMatch supports conditional profile reads.
	IfNoneMatch string `header:"If-None-Match,omitempty"`
}
...
type AdminPlayerCreateRequest struct {
	...
	Username string `json:"username" required:"true" minLength:"3" maxLength:"15" example:"alice"`
...
type AdminPlayerResponse struct {
	...
	// Online reports whether the player currently has an authenticated hotel session.
	Online bool `json:"online" required:"true"`
	// CurrentRoomID identifies the room currently occupied by the online player.
	CurrentRoomID *int64 `json:"currentRoomId,omitempty"`
...
func adminPlayerOperations() []operation {
	return []operation{
		...
		adminPlayer(http.MethodGet, "/api/admin/players/{id}/settings", "Read player client settings", &AdminPlayerPathRequest{}, &AdminPlayerSettingsResponse{}, http.StatusOK),
...
func adminPlayer(method string, path string, summary string, request any, body any, status int) operation {
	...
	return operation{method: method, path: path, tag: "Players", summary: summary,
