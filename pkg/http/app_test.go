...
func TestStatusRouteIsPublic(t *testing.T) {
	...
	if !strings.Contains(body, `"version":"v1.0.0"`) {
		t.Fatalf("expected semantic version body, got %s", body)
	}
}
...
// TestPluginRoutesUsePrivateAPIKey verifies plugin routes share host authentication.
func TestPluginRoutesUsePrivateAPIKey(t *testing.T) {
	registry := pluginroutes.New()
	if err := registry.Mount("demo", func(router fiber.Router) {
		router.Get("/health", func(ctx *fiber.Ctx) error { return ctx.SendStatus(fiber.StatusOK) })
	}); err != nil {
		t.Fatalf("mount plugin route: %v", err)
	}
	app := fiber.New(fiber.Config{ErrorHandler: errorHandler})
	app.Use(auth("secret"))
	registry.Register(app, zap.NewNop())
	response := testRequest(t, app, stdhttp.MethodGet, "/plugins/demo/health")
	if response.StatusCode != fiber.StatusUnauthorized {
		t.Fatalf("expected plugin auth status 401, got %d", response.StatusCode)
	}
	request := newRequest(stdhttp.MethodGet, "/plugins/demo/health")
	request.Header.Set(apiKeyHeader, "secret")
	response, err := app.Test(request)
	if err != nil || response.StatusCode != fiber.StatusOK {
		t.Fatalf("expected authenticated plugin route, status=%d err=%v", response.StatusCode, err)
	}
}
...
func testInfo() build.Info {
	return build.NewInfo("pixels", "v1.0.0", "1234567890")
}
