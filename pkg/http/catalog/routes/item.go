...
func itemPatch(request ItemPatchRequest) catalogadmin.ItemPatch {
	...
	var petTypeID **int32
	var rewardKind *catalogmodel.RewardKind
	...
	if request.PetTypeID != nil {
		petTypeID = &request.PetTypeID
	} else if request.ClearPetType {
		var cleared *int32
		petTypeID = &cleared
	}
	if request.RewardKind != nil {
		value := catalogmodel.RewardKind(*request.RewardKind)
		rewardKind = &value
	}
