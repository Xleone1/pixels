...
// TestReadProjectsLivePresence verifies runtime online and room state are not inferred from persistence.
func TestReadProjectsLivePresence(t *testing.T) {
	...
	peer, err := playerlive.NewSessionPeer("ws-7", "websocket", time.Now())
	if err != nil {
		t.Fatalf("create peer: %v", err)
	}
	player, err := playerlive.NewPlayer(playerlive.Snapshot{ID: 7, Username: "pixel"}, peer)
	if err != nil {
		t.Fatalf("create live player: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("read live player: %v", err)
	}
	body, _ := io.ReadAll(response.Body)
	if response.StatusCode != fiber.StatusOK ||
		!strings.Contains(string(body), `"online":true`) ||
		!strings.Contains(string(body), `"currentRoomId":9`) {
		t.Fatalf("unexpected live response status=%d body=%s", response.StatusCode, body)
	}
}
