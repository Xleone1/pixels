...
func (service *Service) CreateGroup(ctx context.Context, params CreateGroupParams) (permissionmodel.Group, error) {
	group := permissionmodel.Group{Name: strings.TrimSpace(params.Name), Weight: params.Weight,
		Prefix: strings.TrimSpace(params.Prefix), PrefixColor: strings.TrimSpace(params.PrefixColor),
		BadgeURL: strings.TrimSpace(params.BadgeURL), RoomEffectID: params.RoomEffectID, ParentGroupID: params.ParentGroupID}
