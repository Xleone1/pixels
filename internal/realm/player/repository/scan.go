...
func scanPlayer(row pgx.Row) (playermodel.Player, error) {
	...
	err := row.Scan(&player.ID, &player.Username, &player.CreatedAt, &player.UpdatedAt, &deletedAt, &player.Version.Version, &lastLoginAt, &lastLogoutAt, &lastSeenAt, &clubLevel, &clubExpiresAt, &player.AllowTrade, &activeEffectID, &player.AchievementScore)
	if err != nil {
