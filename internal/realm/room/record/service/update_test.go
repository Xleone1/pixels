...
func TestApplyAndValidateUpdateCoversEditableFields(t *testing.T) {
	...
	allowWalk, allowPets, allowEat, hideWalls, hideWired := true, false, true, true, true
	...
	applyUpdate(&room, UpdateParams{Name: &name, Description: &description, CategoryID: &category, MaxUsers: &maxUsers,
		...
		AllowPetsEat: &allowEat, HideWalls: &hideWalls, HideWired: &hideWired, WallThickness: &wall, FloorThickness: &floor,
	...
	if room.Name != name || room.CategoryID == nil || room.MaxUsers != 50 || room.TradeMode != trade || room.RollerSpeed != rollerSpeed || !room.AllowWalkthrough || room.AllowPets || !room.AllowPetsEat || !room.HideWalls || !room.HideWired || room.ChatDistance != 40 || room.ModerationBan != moderation {
