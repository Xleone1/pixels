...
func (service *Service) ListByIDs(ctx context.Context, ids []int64) ([]roommodel.Room, error) {
	if len(ids) == 0 {
		return []roommodel.Room{}, nil
	}
	reader, ok := service.store.(BatchRoomReader)
	if !ok {
		return nil, fmt.Errorf("room store does not support batch reads")
	}
	return reader.ListRoomsByIDs(ctx, ids)
}
...
// ListByOwnerIDs lists active rooms for several owners in one query.
func (service *Service) ListByOwnerIDs(ctx context.Context, ownerPlayerIDs []int64) ([]roommodel.Room, error) {
	if len(ownerPlayerIDs) == 0 {
		return []roommodel.Room{}, nil
	}
	reader, ok := service.store.(OwnerBatchRoomReader)
	if !ok {
		return nil, fmt.Errorf("room store does not support owner batch reads")
	}
	return reader.ListRoomsByOwnerIDs(ctx, ownerPlayerIDs)
}

// ListTagsByRoomIDs lists normalized tags for several rooms in one query.
func (service *Service) ListTagsByRoomIDs(ctx context.Context, roomIDs []int64) (map[int64][]roommodel.Tag, error) {
	if len(roomIDs) == 0 {
		return map[int64][]roommodel.Tag{}, nil
	}
	reader, ok := service.store.(BatchTagReader)
	if !ok {
		return nil, fmt.Errorf("room store does not support tag batch reads")
	}
	return reader.ListRoomTagsByRoomIDs(ctx, roomIDs)
}
...
// ListOfficial lists active staff-picked official rooms.
func (service *Service) ListOfficial(ctx context.Context, limit int) ([]roommodel.Room, error) {
	reader, ok := service.store.(OfficialRoomReader)
	if !ok {
		return nil, fmt.Errorf("room store does not support official room reads")
	}
	return reader.ListOfficialRooms(ctx, normalizeLimit(limit))
}
