...
import (
	...
	outspectator "github.com/niflaot/pixels/networking/outbound/room/spectator"
...
// GroupPolicy prepares and resolves social-group room capabilities.
type GroupPolicy interface {
	// PrepareRoom warms one room group generation.
	PrepareRoom(context.Context, int64) error
	// PreparePlayer warms one player membership generation.
	PreparePlayer(context.Context, int64) error
	// Favorite returns unit projection data for one player.
	Favorite(int64) (int64, int32, string)
	// Furniture returns warmed group identity for one linked furniture item.
	Furniture(int64) (furnituremodel.GroupData, bool)
}

// PromotionProjector sends one room's active purchased event banner.
type PromotionProjector interface {
	// SendActive projects an active promotion to a joining connection.
...
type Handler struct {
	...
	// Groups prepares social-group snapshots before rights projection.
	Groups GroupPolicy
...
func (handler Handler) Handle(ctx context.Context, envelope command.Envelope[Command]) error {
	...
	if handler.Groups != nil {
		...
		if err := handler.Groups.PreparePlayer(ctx, player.ID()); err != nil {
			return err
		}
	}
	...
	occupant, found := active.Occupant(player.ID())
	if found && occupant.Spectator {
		packet, encodeErr := outspectator.Encode()
		if encodeErr != nil {
			return encodeErr
		}
	...
	if handler.Promotions != nil {
		if err := handler.Promotions.SendActive(ctx, room.ID, envelope.Command.Handler); err != nil {
			return err
		}
	}
