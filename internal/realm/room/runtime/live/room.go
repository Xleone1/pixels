...
// FurniturePolicy resolves external room-scoped furniture capabilities.
type FurniturePolicy func(roomID int64, playerID int64) bool
...
type Room struct {
	...
	// furniturePolicy resolves external group decoration rights.
...
// JoinSpectator adds an invisible overflow occupant without a world unit.
func (room *Room) JoinSpectator(occupant Occupant) (Occupancy, error) {
	occupant.Spectator = true
	return room.JoinWithCapacity(occupant, true)
}
...
func (room *Room) JoinWithCapacity(occupant Occupant, bypassCapacity bool) (Occupancy, error) {
	...
	if _, exists := room.occupants[occupant.PlayerID]; !exists && room.occupancyLocked().Count >= room.snapshot.MaxUsers && !bypassCapacity {
		return Occupancy{}, ErrRoomFull
	}
	room.occupants[occupant.PlayerID] = occupant.WithJoinTime(time.Now())
	if room.world != nil && !occupant.Spectator {
...
func (room *Room) CanManageFurniture(playerID int64) bool {
	...
	room.mutex.RLock()
	policy := room.furniturePolicy
	roomID := room.snapshot.ID
	room.mutex.RUnlock()
	return policy != nil && policy(roomID, playerID)
}

// SetFurniturePolicy replaces the external furniture capability resolver.
func (room *Room) SetFurniturePolicy(policy FurniturePolicy) {
	room.mutex.Lock()
	room.furniturePolicy = policy
	room.mutex.Unlock()
}
