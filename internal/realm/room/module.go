...
import (
	...
	wiredwiring "github.com/niflaot/pixels/internal/realm/room/world/wired/wiring"
...
// NewPromotionService creates transactional Room Ads behavior.
func NewPromotionService(config roompromotion.Config, store roompromotion.Store, rooms service.Manager, catalog catalogservice.Manager, permissions permissionservice.Checker, runtime *roomlive.Registry, connections *netconn.Registry) *roompromotion.Service {
	return roompromotion.New(config, store, rooms, catalog, permissions, PromotionManageAny, runtime, connections)
}

// NewGameScoreStore exposes PostgreSQL room game history through its domain contract.
func NewGameScoreStore(repository *roomgamedb.Repository) roomgames.ScoreStore { return repository }
...
// NewRoomService creates room persistence behavior with committed events.
func NewRoomService(store service.Store, layouts layout.Manager, events bus.Publisher) *service.Service {
	return service.New(store, layouts).WithEvents(events)
}
...
// NewBundleStore creates room bundle persistence behavior.
