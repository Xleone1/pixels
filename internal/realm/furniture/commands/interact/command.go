...
func (handler Handler) Handle(ctx context.Context, envelope command.Envelope[Command]) error {
	...
	if handler.Essentials != nil {
		handled, essentialErr := handler.Essentials.Use(ctx, essential.Request{PlayerID: player.ID(), Room: active, Item: item, State: envelope.Command.State, Target: envelope.Command.Handler})
