...
type Service struct {
	...
	// tradePerkRequired enables the optional progression gate.
...
// SetProgressionGate configures the optional permission-backed trade perk.
func (service *Service) SetProgressionGate(required bool, node permission.Node) {
	service.tradePerkRequired = required
	service.tradePerk = node
}
...
func (service *Service) Start(ctx context.Context, actorID int64, targetUnitID int64, actorIP string) (*traderuntime.Session, error) {
	...
	if service.tradePerkRequired && !bypass {
		actorAllowed, err := service.permissions.HasPermission(ctx, actorID, service.tradePerk)
		if err != nil {
			return nil, err
		}
		if !actorAllowed {
		...
		targetAllowed, err := service.permissions.HasPermission(ctx, targetUnit.PlayerID, service.tradePerk)
		if err != nil {
			return nil, err
		}
		if !targetAllowed {
