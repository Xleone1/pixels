...
const (
	...
	CreditsType int32 = -1
	// RewardFurniture identifies the legacy furniture catalog reward.
	RewardFurniture RewardKind = "furniture"
	// RewardPet identifies a pet catalog reward.
...
type Item struct {
	...
	// RewardKind identifies the durable reward type.
	RewardKind RewardKind

	// PetTypeID identifies the renderer species granted by a pet offer.
	PetTypeID *int32

	// PetProductCode identifies Nitro's localized pet product data.
	PetProductCode string
...
// IsPet reports whether the offer grants one pet.
func (item Item) IsPet() bool { return item.RewardKind == RewardPet }
