...
// TestOfferMapsTypedPetProduct verifies Nitro's pet layout discriminator and metadata.
func TestOfferMapsTypedPetProduct(t *testing.T) {
	typeID := int32(15)
	item := catalogmodel.Item{Base: sharedmodel.Base{Identity: sharedmodel.Identity{ID: 11}}, RewardKind: catalogmodel.RewardPet, PetTypeID: &typeID, PetProductCode: "pet15", Amount: 1}
	result, err := OfferProducts(item, nil, nil)
	if err != nil || !result.Pet || result.LocalizationID != "pet15" || len(result.Products) != 1 || result.Products[0].Type != "p" || result.Products[0].ClassID != 15 || result.Products[0].ExtraData != "pet15" {
		t.Fatalf("unexpected pet offer %#v error %v", result, err)
	}
}
