...
func TestTicketAuthenticates(t *testing.T) {
	...
	if len(*sent) != 8 || (*sent)[4].Header != outsettings.Header || (*sent)[5].Header != outhomeroom.Header || (*sent)[6].Header != outnoobness.Header {
		t.Fatalf("expected persisted settings, home room, and retired NUX compatibility packets, got %#v", *sent)
	}
}
