...
func TestAddFavoriteExecutesInsert(t *testing.T) {
	...
	err := New(executor).AddFavorite(context.Background(), 7, 9, 30, false)
	if err != nil {
...
func TestSaveSearchScansRecord(t *testing.T) {
	search, err := New(&fakeExecutor{row: fakeRow{values: savedSearchValuesForTest()}}).SaveSearch(context.Background(), navmodel.SaveSearchParams{PlayerID: 7, Code: "hotel_view"})
	if err != nil {
...
func TestUpsertPreferenceScansRecord(t *testing.T) {
	preference, err := New(&fakeExecutor{row: fakeRow{values: preferenceValuesForTest()}}).UpsertPreference(context.Background(), navmodel.PreferenceParams{})
	if err != nil {
...
func TestUpsertCategoryPreferenceScansRecord(t *testing.T) {
	preference, err := New(&fakeExecutor{row: fakeRow{values: categoryPreferenceValuesForTest()}}).UpsertCategoryPreference(context.Background(), navmodel.CategoryPreferenceParams{})
	if err != nil {
