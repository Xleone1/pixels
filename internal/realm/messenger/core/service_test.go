...
// TestCardsCachesDurableDataAndRefreshesLivePresence verifies cache hits avoid player reads without freezing online state.
func TestCardsCachesDurableDataAndRefreshesLivePresence(t *testing.T) {
	service, store := serviceFixture(t)
	store.friends[[2]int64{1, 2}] = true
	first, err := service.Cards(context.Background(), 1)
	if err != nil || len(first) != 1 || first[0].Online {
		t.Fatalf("unexpected first cards=%#v err=%v", first, err)
	}
	delete(service.players.(*fakePlayers).records, 2)
	addLivePlayer(t, service.live, 2, "alice", 9)
	second, err := service.Cards(context.Background(), 1)
	if err != nil || len(second) != 1 || !second[0].Online || !second[0].FollowingAllowed {
		t.Fatalf("unexpected cached cards=%#v err=%v", second, err)
	}
}
...
func serviceFixture(test testTB) (*Service, *fakeStore) {
	...
	service := New(Options{MaxFriends: 200, MaxFriendsClub: 500, MaxSearchResults: 50, SearchCacheTTL: time.Minute, FriendCacheTTL: time.Minute, SearchThrottle: time.Second, ChatThrottle: time.Millisecond}, store, players, playerlive.NewRegistry(), roomlive.NewRegistry(nil), nil, client, nil, Nodes{}, nil)
